...
int main(int argc, char *argv[]) {
  ...
  dfs(hdr->BPB_RootClus, 0, 1); //遍历根目录
...
void dfs(u32 clusId, int depth, int is_dir) {
  ...
  for (; clusId < CLUS_INVALID; clusId = next_cluster(clusId)) {
    // 簇ID 是有效的 就进入下一个簇  遍历链表 
    if (is_dir) {
      int ndents = hdr->BPB_BytsPerSec * hdr->BPB_SecPerClus / sizeof(struct fat32dent); //获得dents的数量
...
//./fatree /media/cjw/work1/nju-os/fs.img 
